...
iterm_re = re.compile(r"(.+)\.itermcolors$")
...
class Converter:
    ...
    def parse_scheme(self, scheme):
        ...
        with open(iterm_path, "rb") as f:
            ...
            # Second loop needed because guint16 palette must have colors in sorted
            # order
            for color_index in range(16):
                key = "Ansi %d Color" % color_index
                guint16_pallete += self.calculate_color_components_guint16(plist[key])

            colors_dict["Guint16_Palette"] = "{{{}}}".format(", ".join(guint16_pallete))
    ...
    def calculate_color_components_guint16(self, color_components):
    ...
    def generate_from_template(self, task_id, colors, template) -> None:
        ...
        for scheme in colors:
            ...
            f = open(destination, "w", encoding="locale")
            f.write(result)
            f.close()
